...
var db = require("../../../db");
var PurchasePriceCorrectionManager = require("dl-module").managers.garmentPurchasing.PurchaseQuantityCorrectionManager;
...
function getRouter(){
    ...
    var handlePdfRequest = function (request, response, next) {
        db.get().then(db => {
            ...
            manager.getPdfReturNotePPn(id, request.timezoneOffset)
                .then(docBinary => {
                    // var base64 = 'data:application/pdf;base64,' + docBinary.toString('base64')
                    manager.getSingleById(id,["returNoteNo"])
                        .then(doc => {
                            response.writeHead(200, {
                                'Content-Type': 'application/pdf',
                                'Content-Disposition': `attachment; filename=${doc.returNoteNo}-ppn.pdf`,
