...
function getRouter() {
    ...
    var getManager = (user) => {
        return db.get()
            .then((db) => {
                return Promise.resolve(new CustomsManager(db, user));
            });
    };
    ...
    router.get("/", passport, function (request, response, next) {
        ...
        getManager(user)
            .then((manager) => {
                customsManager = manager;
                return customsManager.getCustomsReport(query);
            })
            .then(docs => {
            ...
            .then((result) => {
                if ((request.headers.accept || '').toString().indexOf("application/xls") < 0) {
                    ...
                    customsManager.getCustomsReportXls(result, query)
                        .then(xls => {
